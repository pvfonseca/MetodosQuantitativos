...
$$\begin{align}
\mathcal{L}_{xx} &= f_{xx} - \lambda g_{xx} \\
\mathcal{L}_{xy} &= f_{xy} - \lambda g_{xy} \\
\mathcal{L}_{yy} &= f_{yy} - \lambda g_{yy}
\end{align}$$

* Portanto:

$$d^2z = \mathcal{L}_{xx}dx^2 + \mathcal{L}_{xy}dxdy + \mathcal{L}_{yx}dydx + \mathcal{L}_{yy}dy^2\tag{4}$$
